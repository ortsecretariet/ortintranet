...
function parseLocalDate(dateStr) {
  ...
  // SQLite localtime strings have no timezone — treat as Africa/Nairobi (UTC+3)
...
function fmtDate(dateStr) {
  if (!dateStr) return '';
  return parseLocalDate(dateStr).toLocaleDateString('en-GB', { day: 'numeric', month: 'long', year: 'numeric' });
}
...
function noticeRowHTML(n) {
  ...
  const officePill = n.issuing_office
    ? `<span class="row-pill" style="background:#f0f4ff;color:#2c5f9e;">🏛️ ${escapeHtml(n.issuing_office)}</span>` : '';
  return `
    <div class="notice-row${n.is_urgent ? ' notice-row-urgent' : ''}" onclick="openDetail(${n.id})" role="button" tabindex="0">
      <div class="notice-row-left">
        <div class="notice-row-pills">${urgentTag}${catPill}${officePill}</div>
...
document.getElementById('unifiedPublishForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const pubOffice = document.getElementById('pub_issuing_office')?.value;
    if (pubOffice) formData.append('issuing_office', pubOffice);
...
document.getElementById('docOfficeFilter').addEventListener('change', renderDocuments);
...
function renderDocuments() {
  ...
  const officeFilter = document.getElementById('docOfficeFilter').value;
  ...
  let docs = visibleDocs.filter(d => {
    ...
    const matchOffice = !officeFilter || d.issuing_office === officeFilter;
    return matchQ && matchType && matchDate && matchOffice;
  });
  ...
  list.innerHTML = docs.map(d => {
    const fileUrl = d.file_url
      ...
      : '#';
    const officeBadge = d.issuing_office
      ? `<span style="font-size:11px;font-weight:600;background:#f0f4ff;color:#2c5f9e;padding:2px 8px;border-radius:999px;">🏛️ ${escapeHtml(d.issuing_office)}</span>`
      : '';
    return `
      ...
      <div class="notice-card-head">
        <div style="display:flex;align-items:center;gap:10px;">
          ...
          <div>
            ...
            <div class="notice-meta" style="margin:4px 0 0;display:flex;flex-wrap:wrap;gap:6px;align-items:center;">
              ${d.is_public
...
document.getElementById('docUploadForm').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    const issuingOffice = document.getElementById('d_issuing_office').value;
    if (issuingOffice) formData.append('issuing_office', issuingOffice);
...
async function init() {
  ...
  if (isAdmin) {
    ...
    document.getElementById('pubOfficeWrap').style.display       = 'block';
  } else {
...
function openPreview(url, name) {
  ...
  const downloadFallback = `
    <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);
      ...
      <p style="color:#555;font-size:15px;margin-bottom:20px;">This file type cannot be previewed in the browser.</p>
      <a href="${url}" target="_blank" download
         style="background:#1E6B44;color:#fff;padding:10px 24px;border-radius:8px;
                text-decoration:none;font-weight:600;font-size:14px;">⬇ Download File</a>
    </div>`;
  ...
  if (['jpg','jpeg','png','gif','webp','svg'].includes(ext)) {
    ...
    // Office files — show download prompt since Google Docs Viewer
    // requires a public internet URL (won't work on local network)
    content.innerHTML = `
      <div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%);
                  text-align:center;padding:30px;">
        <div style="font-size:48px;margin-bottom:16px;">
          ${ ext.includes('doc') ? '📝' : ext.includes('xls') ? '📊' : '📑' }
        </div>
        <p style="color:#333;font-size:15px;font-weight:600;margin-bottom:6px;">${escapeHtml(name)}</p>
        <p style="color:#777;font-size:13px;margin-bottom:20px;">
          Office documents must be downloaded to view.<br>
          Open with Microsoft Office or Google Docs after downloading.
        </p>
        <a href="${url}" target="_blank" download
           style="background:#1E6B44;color:#fff;padding:10px 24px;border-radius:8px;
                  text-decoration:none;font-weight:600;font-size:14px;">⬇ Download to View</a>
      </div>`;

  } else if (ext === 'txt' || ext === 'csv') {
    // Plain text — fetch and display inline
    fetch(url)
      .then(r => r.text())
      .then(text => {
        const pre = document.createElement('pre');
        pre.style.cssText = 'padding:20px;font-size:13px;line-height:1.6;white-space:pre-wrap;word-break:break-word;color:#333;';
        pre.textContent = text;
        content.appendChild(pre);
      })
      .catch(() => { content.innerHTML = downloadFallback; });

  } else {
    content.innerHTML = downloadFallback;
  }
